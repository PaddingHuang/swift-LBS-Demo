...
import UIKit
import Alamofire
class MultiRoutePlanViewController: UIViewController, MAMapViewDelegate, AMapNaviDriveManagerDelegate, DriveNaviViewControllerDelegate, BottomInfoViewDelegate,AMapLocationManagerDelegate{
    ...
    var gpsArr:[KindsPointAnnotation]?
                ...
                if let value = resp.result.value{
                    ...
                    if code == 0{
                        ...
                        for mod in result!{
                            ...
                            let annotation = KindsPointAnnotation()
                            ...
                            annotation.title = model.RoadName!
                            annotation.gpsModel = model
                           
                            self.gpsArr?.append(annotation)
...
//    
   ...
   //返回按钮
    ...
    func initRoadBtn(){
        ...
        roadBtn.frame = CGRect.init(x: self.view.frame.size.width-13-30, y: 50, width: 35, height: 35)
        roadBtn.backgroundColor = UIColor.white
        roadBtn.setImage(UIImage.init(named: "map_traffic_white"), for: .normal)
        roadBtn.setImage(UIImage.init(named: "map_traffic_hl_white"), for: .selected)
        roadBtn.isSelected = true
        roadBtn.backgroundColor = UIColor.init(colorLiteralRed: 139/255.0, green: 139/255.0, blue: 139/255.0, alpha: 1)
        
        roadBtn.addTarget(self, action: #selector(roadBtnClick(_:)), for: .touchUpInside)
    }
    
    func initSelectBtn(){
        let selectBtn = UIButton()
        self.view.addSubview(selectBtn)
        selectBtn.frame = CGRect.init(x: 10, y: self.view.frame.size.height-220, width: 35, height: 35)
        selectBtn.backgroundColor = UIColor.white
        selectBtn.setImage(UIImage.init(named: "route_car_result_prefer_icon_normal"), for: .normal)
        selectBtn.setImage(UIImage.init(named: "route_car_result_prefer_icon_selected"), for: .selected)
        
        selectBtn.backgroundColor = UIColor.init(colorLiteralRed: 139/255.0, green: 139/255.0, blue: 139/255.0, alpha: 1)
    ...
    func roadBtnClick(_ btn:UIButton){
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.locationManager.distanceFilter = 20000
    ...
    func initMapView() {
        mapView = MAMapView(frame: CGRect(x: 0, y: 0, width: view.bounds.width, height: view.bounds.height  - bottomInfoViewHeight))
    ...
    func createRouteTagString() -> [NSNumber: String]? {
        ...
        let strategy = preferenceView.strategy(isMultiple: true)
    ...
    func configSubview() {
        ...
        preferenceView = PreferenceView(frame: CGRect(x: 0, y: 0, width: 0, height: 0))
        view.addSubview(preferenceView)
        
//        let routeBtn = buttonForTitle("路径规划")
//        routeBtn.frame = CGRect(x:0, y: 0, width: 0, height: 0)
//        routeBtn.addTarget(self, action: #selector(self.routePlanAction(sender:)), for: .touchUpInside)
//        
//        view.addSubview(routeBtn)
        
        bottomInfoView = BottomInfoView(frame: CGRect(x: 0, y: view.bounds.height  - bottomInfoViewHeight, width: view.bounds.width, height: bottomInfoViewHeight))
    ...
    func amapLocationManager(_ manager: AMapLocationManager!, didUpdate location: CLLocation!) {
        ...
        let Latitude = CGFloat((self.location.coordinate.latitude))
        let longitude = CGFloat((self.location.coordinate.longitude))
        
        self.startPoint = AMapNaviPoint.location(withLatitude: Latitude, longitude: longitude)!
        self.endPoint = AMapNaviPoint.location(withLatitude: model.location.latitude, longitude: model.location.longitude)!
initAnnotations()
        if needRoutePlan {
    ...
    func mapView(_ mapView: MAMapView!, viewFor annotation: MAAnnotation!) -> MAAnnotationView! {
        ...
        if annotation is NaviPointAnnotation {
            ...
            if annotation.naviPointType == .start {
                pointAnnotationView?.image = UIImage.init(named: "startPoint")
            }
            else if annotation.naviPointType == .end {
                pointAnnotationView?.image = UIImage.init(named: "endPoint")
            }
            
            return pointAnnotationView
        }else  if annotation.isKind(of: KindsPointAnnotation.self) {
            ...
            if   ann.gpsModel?.Levels == "1"{
                annotationView?.image = UIImage.init(named: "camera_gray")
            }else  if   ann.gpsModel?.Levels == "2"{
                annotationView?.image = UIImage.init(named: "camera_blue")
            }else  if   ann.gpsModel?.Levels == "3" {
                annotationView?.image = UIImage.init(named: "camera_yellow")
            }else  if   ann.gpsModel?.Levels == "4" {
                annotationView?.image = UIImage.init(named: "camera_red")
            }
